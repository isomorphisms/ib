...
Idriç/Grease runtime boundary.  Current IB documentation already calls for
independent category decisions, positive/unlabeled treatment, durable source
identity, and rebuildable derived state.  The repository also contains Idriç
numeric/vector work that makes an affine separator a plausible eventual typed
boundary, but there is no current executable Idriç conversation classifier on
`main`.  This experiment therefore uses Python/scikit-learn as an explicit
empirical oracle and must not be reported as Idriç execution.
...
On 2026-09-14 the available saved-file Library was searched directly for
`conversations.json`, ChatGPT export ZIP/HTML artifacts, and equivalent archive
names; none was present.  Connected repositories were also searched for an
already staged `conversations.json` or conversation archive and none was found.
This closes the remaining cheap bulk-acquisition avenues currently exposed to
this environment without going back to per-conversation account traversal.
...
roles and conversations with detected child-identifying context or actual
third-party correspondence-shaped source text.  Merely asking to draft, send,
or reply to a message is not a whole-conversation exclusion.  The importer
redacts credentials, secret-bearing URLs, financial/government identifiers,
exact street addresses, email addresses, and telephone numbers.  Each removal
produces a mechanical record containing a reason, location, original byte
length, and one-way hash—not the removed text.
...
corpus/
  ...
  definitions/         category definitions plus separate filing policies
...
## Filing is a policy projection, not another name for concept classification

Accepted concepts are projected through a separate versioned filing policy
conforming to `corpus/definitions/filing-policy.schema.json`.  The policy maps
one or more concepts to named ChatGPT destinations and may give weak inherited
location a small explicit weight.  Its support score is deterministic policy
support, not a probability.

Per-category classifier margins are retained in the explanation record but are
not summed across independently calibrated category planes.  Filing can return
a confident destination, several plausible destinations, or no sufficiently
supported destination.  Explicit filing corrections remain authoritative and
can override or block policy output without rewriting the underlying concept
proposal.

A direct `filing_destination` classifier remains available only as a diagnostic
baseline.  It must be reported separately from the primary concept-to-filing
policy path.
...
```text
import-export -> partition -> train-model -> classify-model -> project -> destinations
```
...
`correct` appends a user correction.  `project` overlays authoritative concept
evidence without rewriting either the source or immutable classifier proposal.
`destinations` consumes resolved concept proposals, a versioned filing policy,
and the evidence log; changing only filing policy does not require retraining.
...
between proposal generations.  This is the deterministic classification and
projection layer only; there is no ChatGPT mover.
