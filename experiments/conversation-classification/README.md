...
fact.  `verify-public` is a required deterministic gate, and remaining
`manual_review_required` findings block publication rather than being waved
through.
...
```text
corpus/
  raw/                 screened source-faithful Markdown by stable corpus id
  metadata/            source identity, chronology, message graph, and hashes
  index.jsonl          inspectable corpus inventory
  safety/
    exclusions.jsonl   whole-conversation and message exclusions
  ...
  views/               rebuildable query projections
...
than summaries.  They are not the unredacted account export.
