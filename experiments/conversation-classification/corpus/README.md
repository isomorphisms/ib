...
`raw/` preserves screened source messages as structured JSON.  `metadata/`
preserves identity and message structure.  Markdown is only a rebuildable
inspection view under `views/source/`; classifiers do not reparse it.
Assertions and corrections are append-only.  Everything under `derived/` and `views/` must be reproducible from raw/metadata,
